...
Still open for Phase 3: the backend's own connection (asyncpg through the Supavisor session pooler, `DATABASE_URL` in `backend/.env`), verified with `python -m scripts.verify_database`.
