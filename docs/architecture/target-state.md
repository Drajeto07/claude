...
Provider abstraction stays and becomes swappable in practice (Anthropic today, OpenAI or another provider without touching business logic). Explicit task separation: Structure Analysis, Instruction Extraction, Style Analysis, Style Mapping (for Format by Example), optional Document Classification. No task is ever "rewrite the document." Every AI call has schema validation, retries, timeout, token budget, telemetry, and a defined error/fallback state. Large documents are chunked (stable IDs per chunk, merged results, global consistency validation) instead of one unbounded prompt. Prompts strictly separate system instructions / user formatting instructions / document content — document content is always untrusted data, never treated as instructions. Done in Phase 15: rules in the system prompt and the document between tags named at random for each call (`ai/prompting.py`); timeouts, SDK retries and a telemetry line per call; long text analysed a piece at a time with the earlier headings as context, a failing piece falling back alone, and a cap on how much goes to the AI (the document says when it was reached).

## Security

Uploads are judged by their bytes (signatures, a declared type that fits, ZIP entry/size/ratio limits before python-docx opens anything, an XML parser that resolves no entities, a PDF page cap); every request body is capped before it is read; sign-in, registration, AI work, uploads and exports are rate-limited (in memory, or shared through Redis); every response carries security headers, the frontend a Content-Security-Policy; CORS names its origins; secrets are hidden settings; security-relevant events are structured audit lines with the request id, and no log line holds document content. Done in Phase 15 (README, "Security and observability").
