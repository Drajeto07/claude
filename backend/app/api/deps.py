...
async def get_metered_ai_provider(
    user: CurrentUser, db: DbSession, provider: Annotated[AIProvider, Depends(get_ai_provider)]
) -> AIProvider:
    """The AI provider for a request that calls it directly, counting each
    completed call into the user's usage. The count is committed with the
    request's own change; an endpoint that changes nothing commits it itself."""
    workspace_id = await AuthService(db).default_workspace_id(user.id)
    return MeteredAIProvider(provider, lambda: db.add(usage_row(workspace_id, AI_OPERATIONS)))
