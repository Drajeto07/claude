...
async def _start(jobs: JobService, queue: Queue, job_type: str, **job) -> JobOut:
...
async def import_text(payload: ImportTextJobRequest, jobs: Jobs, queue: Queue) -> JobOut:
    """Pasted text into a new document (structure analysis, AI for plain prose)."""
    return await _start(jobs, queue, IMPORT_TEXT, payload={"text": payload.text, "title": payload.title})


@router.post("/import-file", response_model=JobOut, status_code=202)
async def import_file(
    jobs: Jobs, queue: Queue, file: UploadFile = File(...), title: Annotated[str | None, Form()] = None
) -> JobOut:
...
async def format_document(
    ...
    payload = {
        ...
        "instructionsText": await instructions_from(instructionsText, instructionsFile),
    ...
    return await _start(jobs, queue, FORMAT, document_id=documentId, payload=payload)


@router.post("/export", response_model=JobOut, status_code=202)
async def export_document(
    payload: ExportJobRequest, user: CurrentUser, db: DbSession, storage: Storage, jobs: Jobs, queue: Queue
) -> JobOut:
    ...
    return await _start(jobs, queue, EXPORT, document_id=payload.documentId, payload=payload.model_dump(exclude={"documentId"}))


@router.post("/extract-reference", response_model=JobOut, status_code=202)
async def extract_reference(jobs: Jobs, queue: Queue, file: UploadFile = File(...)) -> JobOut:
