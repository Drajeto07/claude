...
def _upload_picture_document(client: TestClient, png: bytes) -> dict:
    response = client.post("/api/v1/documents/upload", files={"file": ("report.docx", _docx_with_picture(png), _DOCX_TYPE)})
...
def test_uploaded_docx_picture_is_stored_as_an_asset_and_served_back(alice):
    ...
    served = alice.get(f"/api/v1/assets/{image['assetId']}")
...
def test_assets_are_private_to_their_workspace(api_db, alice):
    ...
    assert error_body(bob.get(f"/api/v1/assets/{asset_id}")) == error_body(bob.get("/api/v1/assets/does-not-exist"))
    assert anonymous.get(f"/api/v1/assets/{asset_id}").status_code == 401


def test_pasted_image_is_stored_once_not_on_every_autosave(api_db, alice):
    document = alice.post("/api/v1/documents", json={"text": "# Notes\n\nSome text to start with here."}).json()
    pasted = {"type": "image", "content": "", "order": 99, "image": {"src": "data:image/png;base64," + base64.b64encode(_png()).decode()}}

    saved = alice.put(f"/api/v1/documents/{document['id']}/content", json={"elements": document["elements"] + [pasted]})
    ...
    alice.put(f"/api/v1/documents/{document['id']}/content", json={"elements": saved.json()["elements"]})
...
def test_inline_image_that_is_not_a_web_image_is_removed_and_reported(api_db, alice):
    ...
    saved = alice.put(f"/api/v1/documents/{document['id']}/content", json={"elements": document["elements"] + [junk]}).json()
...
def test_docx_and_pdf_exports_embed_the_stored_picture(alice):
    ...
    docx_bytes = alice.get(f"/api/v1/documents/{document['id']}/export/docx").content
    pdf_bytes = alice.get(f"/api/v1/documents/{document['id']}/export/pdf").content
...
def test_a_planted_foreign_asset_id_does_not_leak_into_an_export(api_db, alice):
    ...
    bob = _signed_in_client("bob@example.com")
    bobs_document = bob.post("/api/v1/documents", json={"text": "# Mine\n\nBob's own paragraph of text."}).json()
    planted = {"type": "image", "content": "", "order": 99, "image": {"src": "", "assetId": alices_asset}}
    bob.put(f"/api/v1/documents/{bobs_document['id']}/content", json={"elements": bobs_document["elements"] + [planted]})

    export = bob.get(f"/api/v1/documents/{bobs_document['id']}/export/docx")
