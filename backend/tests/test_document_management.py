...
def signed_in(api_db):
    ...
    assert client.post("/api/auth/register", json={"email": "manager@example.com", "password": "long enough password"}).status_code == 201
    yield
    client.cookies.clear()


def _create(title: str, text: str = _TEXT) -> dict:
    return client.post("/api/documents", json={"text": text, "title": title}).json()
...
def test_the_list_shows_the_newest_first_with_what_each_document_is():
    first = _create("First")
    uploaded = client.post("/api/documents/upload", files={"file": ("Plan.docx", _docx(), _DOCX)}, data={"title": "Plan"}).json()
    client.post(f"/api/documents/{first['id']}/format", data={"templateId": "modern-report"})
...
def test_the_list_searches_titles_sorts_and_pages():
    ...
    assert {item["title"] for item in client.get("/api/documents", params={"q": "BUDGET"}).json()["items"]} == {"Budget 2026", "budget notes"}
    # % and _ are searched for as themselves, not as wildcards.
    assert [item["title"] for item in client.get("/api/documents", params={"q": "0% d"}).json()["items"]] == ["100% done_ok"]
    assert client.get("/api/documents", params={"q": "_"}).json()["total"] == 1

    by_title = client.get("/api/documents", params={"sort": "title"}).json()["items"]
    assert [item["title"] for item in by_title] == ["100% done_ok", "Budget 2026", "budget notes", "Minutes"]

    page = client.get("/api/documents", params={"sort": "title", "limit": 2, "offset": 2}).json()
...
def test_nobody_sees_anyone_elses_documents_in_the_list():
    _create("Mine")
    client.cookies.clear()
    client.post("/api/auth/register", json={"email": "someone@example.com", "password": "long enough password"})

    assert client.get("/api/documents").json() == {"items": [], "total": 0}


def test_versions_say_what_each_change_did_and_which_is_current():
    document = _create("Versions")
    client.patch(f"/api/documents/{document['id']}", json={"title": "Renamed"})
    client.post(f"/api/documents/{document['id']}/format", data={"templateId": "academic-default"})
    ...
    assert [v["current"] for v in client.get(f"/api/documents/{document['id']}/versions").json()] == [False, True, False]


def test_an_old_version_can_be_looked_at_and_restored():
    document = _create("Original title")
    client.patch(f"/api/documents/{document['id']}", json={"title": "Second title"})

    original = client.get(f"/api/documents/{document['id']}/versions/1").json()
    assert original["metadata"]["title"] == "Original title"
    assert original["resolvedStyles"]  # worked out, ready to show
    assert client.get(f"/api/documents/{document['id']}/versions/9").status_code == 404

    current = client.get(f"/api/documents/{document['id']}").json()
    stale = client.post(f"/api/documents/{document['id']}/versions/1/restore", headers={"If-Match": str(current["revision"] - 1)})
    assert stale.status_code == 412

    restored = client.post(f"/api/documents/{document['id']}/versions/1/restore", headers={"If-Match": str(current["revision"])})
    assert restored.status_code == 200 and restored.json()["metadata"]["title"] == "Original title"
    top = client.get(f"/api/documents/{document['id']}/versions").json()[0]
    ...
    assert client.post(f"/api/documents/{document['id']}/undo").json()["metadata"]["title"] == "Second title"


def test_before_and_after_compares_the_original_with_now():
    document = _create("Compare")
    client.post(f"/api/documents/{document['id']}/format", data={"templateId": "academic-default"})
    now = client.get(f"/api/documents/{document['id']}").json()
    ...
    client.patch(f"/api/documents/{document['id']}/settings", json={"property": "marginTop", "value": "3", "unit": "cm"})

    comparison = client.get(f"/api/documents/{document['id']}/compare").json()
    ...
    between = client.get(f"/api/documents/{document['id']}/compare", params={"from": 2, "to": 3}).json()
    assert between["structure"] and not between["settings"]
    assert client.get(f"/api/documents/{document['id']}/compare", params={"from": 7}).status_code == 404


def test_deleting_is_for_good_and_takes_the_history_along():
    document = _create("Doomed")
    client.patch(f"/api/documents/{document['id']}", json={"title": "Still doomed"})
    ...
    assert client.get(f"/api/documents/{document['id']}/versions").status_code == 404
