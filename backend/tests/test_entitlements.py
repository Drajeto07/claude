...
def signed_in(api_db):
    ...
    assert client.post("/api/v1/auth/register", json={"email": "plan@example.com", "password": "long enough password"}).status_code == 201
    yield
    client.cookies.clear()
...
def _document() -> str:
    response = client.post("/api/v1/documents", json={"text": _MARKDOWN})
...
def test_the_document_limit_stops_new_documents_however_they_are_made(monkeypatch, api_db):
    ...
    body = _refused(client.post("/api/v1/documents", json={"text": _MARKDOWN}), "maxDocuments")
    assert (body["details"]["limit"], body["details"]["used"]) == (1, 1)
    assert "1 document." in body["message"]
    _refused(client.post("/api/v1/documents/upload", files={"file": ("a.txt", b"Some text.", "text/plain")}), "maxDocuments")
    _refused(client.post("/api/v1/jobs/import-text", json={"text": _MARKDOWN}), "maxDocuments")
    _refused(client.post("/api/v1/jobs/import-file", files={"file": ("a.docx", _docx(), _DOCX)}), "maxDocuments")
    ...
    assert client.post("/api/v1/jobs/import-text", json={"text": _MARKDOWN}).json()["status"] == "succeeded"
...
def test_an_export_the_plan_doesnt_include_is_refused(monkeypatch):
    ...
    body = _refused(client.get(f"/api/v1/documents/{document_id}/export/pdf"), "canExportPdf")
    assert "PDF" in body["message"]
    _refused(client.post("/api/v1/jobs/export", json={"documentId": document_id, "format": "pdf"}), "canExportPdf")
    assert client.get(f"/api/v1/documents/{document_id}/export/docx").status_code == 200
    assert client.post("/api/v1/jobs/export", json={"documentId": document_id, "format": "docx"}).json()["status"] == "succeeded"
...
def test_a_file_over_the_plans_size_is_refused_before_it_is_read(monkeypatch):
    ...
    body = _refused(client.post("/api/v1/documents/upload", files={"file": ("big.txt", big, "text/plain")}), "maxDocumentSizeMb")
    assert (body["details"]["limit"], body["details"]["used"]) == (1, 2)
    _refused(client.post("/api/v1/jobs/import-file", files={"file": ("big.docx", big, _DOCX)}), "maxDocumentSizeMb")
    _refused(client.post("/api/v1/jobs/extract-reference", files={"file": ("big.docx", big, _DOCX)}), "maxDocumentSizeMb")
    _refused(client.post("/api/v1/templates/extract", files={"file": ("big.docx", big, _DOCX)}), "maxDocumentSizeMb")
    assert client.post("/api/v1/documents/upload", files={"file": ("small.txt", b"Small enough.", "text/plain")}).status_code == 201
...
def test_used_up_ai_operations_refuse_work_that_is_all_ai(monkeypatch, api_db):
    ...
    first = client.post("/api/v1/jobs/format", data={"documentId": document_id, "instructionsText": "make the text bold"}).json()
    assert first["result"]["status"] == "applied"

    body = _refused(client.post("/api/v1/jobs/format", data={"documentId": document_id, "instructionsText": "make it blue"}), "maxAiOperations")
    assert (body["details"]["limit"], body["details"]["used"]) == (1, 1)
    _refused(client.post(f"/api/v1/documents/{document_id}/format", data={"instructionsText": "make it blue"}), "maxAiOperations")
    _refused(client.post(f"/api/v1/documents/{document_id}/style-analysis"), "maxAiOperations")
    # A template needs no AI, so it still applies.
    assert client.post("/api/v1/jobs/format", data={"documentId": document_id, "templateId": "academic-default"}).json()["status"] == "succeeded"
...
def test_used_up_ai_operations_make_the_optional_ai_steps_fall_back(monkeypatch):
    ...
    assert client.post("/api/v1/jobs/import-text", json={"text": _PROSE}).json()["status"] == "succeeded"
    assert client.post("/api/v1/documents", json={"text": _PROSE}).status_code == 201
    assert fake.calls == 0


@pytest.mark.parametrize("path", ["/api/v1/jobs/import-text", "/api/v1/documents"])
...
def test_the_template_limit_stops_new_templates(monkeypatch):
    ...
    assert client.post("/api/v1/templates", json={"name": "House style"}).status_code == 201

    body = _refused(client.post("/api/v1/templates", json={"name": "Another"}), "maxTemplates")
...
def test_storage_over_the_plan_is_refused_before_anything_is_stored(monkeypatch, api_db, tmp_path):
    ...
    body = _refused(client.post("/api/v1/documents/upload", files={"file": ("photo.docx", _docx(picture=photo), _DOCX)}), "maxStorageMb")
    assert body["details"]["limit"] == 1
    document = client.get(f"/api/v1/documents/{_document()}").json()
    pasted = {"type": "image", "content": "", "order": 99, "image": {"src": "data:image/png;base64," + base64.b64encode(photo).decode()}}
    _refused(client.put(f"/api/v1/documents/{document['id']}/content", json={"elements": document["elements"] + [pasted]}), "maxStorageMb")
    with OrmSession(api_db) as session:
...
def test_jobs_of_a_plan_with_priority_processing_are_queued_ahead(monkeypatch, api_db):
    ...
    client.post("/api/v1/jobs/import-text", json={"text": _MARKDOWN})
    with OrmSession(api_db) as session:
