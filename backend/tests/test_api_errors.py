...
def signed_in(api_db):
    ...
    assert client.post("/api/auth/register", json={"email": "errors@example.com", "password": "long enough password"}).status_code == 201
    yield
    client.cookies.clear()


def _document() -> dict:
    return client.post("/api/documents", json={"text": "# Title\n\nA paragraph long enough to be one."}).json()
...
def test_signed_out_and_foreign_origin_errors_say_which_they_are():
    client.cookies.clear()
    assert client.get("/api/documents/missing").json()["code"] == "not_signed_in"

    refused = client.post("/api/auth/logout", headers={"Origin": "https://evil.example"})
...
def test_a_validation_error_names_the_fields_but_never_echoes_what_was_sent():
    client.cookies.clear()
    response = client.post("/api/auth/register", json={"email": "not-an-email", "password": "s3cr3t"})
...
def test_a_revision_conflict_says_which_revision_is_current():
    ...
    response = client.patch(f"/api/documents/{document['id']}", json={"title": "New"}, headers={"If-Match": "999"})
...
def test_an_unexpected_failure_is_a_plain_500_with_the_request_id(monkeypatch):
    ...
    response = client.get("/api/documents/any", headers={"X-Request-ID": "trace-123"})
